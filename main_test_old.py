...
def construct_collectives(
    ...
    logged_gpus = defaultdict(int)
    logged_communicator = None
    coll_info = coll_info.sort_values(["start"], ascending=[True])
    for _, row in coll_info.iterrows():
        if logged_gpus[row['gpu']] <= 1 and (row['commId'] == logged_communicator or logged_communicator is None):
            if row['parallelism'] == "DP":
            #     print(row["data_size"])
            # if row["collective"] == "AllReduce":
                row['gpu'].add_collective(row['stream'], row['collOp'], row['start'], row['end'], row['context_label'])
                logged_gpus[row['gpu']] += 1
...
#%%
    ...
    # gpu2goal_rank = {gpu: i for i, gpu in enumerate(gpu_devices.values()) if len(gpu.streams) > 0}
    ...
    with open("trace_double_tree.goal", "w") as f:
        logger.info("writing goal file")
